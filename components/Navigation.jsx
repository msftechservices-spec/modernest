...
export default function Navigation() {
    ...
    return (
        <>
            <button className="block lg:hidden text-text-primary" onClick={() => setOpen(!isOpen)}>
                <svg
            ...
            <nav className="md:flex hidden items-center gap-12 text-lg text-text-primary">
                {nav.map(({ href, label }) => (
                    <Link key={href} href={href} className={
                        pathname === href ? 'text-accent font-semibold' : 'hover:text-accent transition-colors'
                    }>
                        {label}
                    </Link>
                ))}
